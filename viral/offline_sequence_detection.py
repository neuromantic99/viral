...
def compute_pse_thresholds_session(
    ...
    # you need to make sure the settings are the same as in the task decoding!!!
    ssp = get_ssp_vectors(
        trials=trials,
...
def grosmark_bayesian_decoder(
    ...
    Returns:
        ...
        np.ndarray:                                 decoded_pos (decoded position for each time bin based on the pr_max; shape: (n_time_bins)).
        np.ndarray:                                 true_pos (true position for each time bin, if positions_test is provided; shape: (n_time_bins)).
...
def climer_bayesian_decoder(
    ...
    *,
    ...
    # TODO: remove after all? at least for the event decoding, there is an assertion guarding this
    # if n_timebins < 10:
    # if (
    #     n_timebins < 3
    ...
    for s in range(n_samples):
        ...
        log_post_shifted = log_post - np.max(log_post, axis=0, keepdims=True)
        post_unnorm = np.exp(log_post_shifted)
        post = post_unnorm / np.sum(post_unnorm, axis=0, keepdims=True)
...
def plot_pse_event(
    ...
    xtick_frames = np.arange(0, n_time * bayesian_config.bin_size_time_offline + 1, 15)
    xtick_bins = xtick_frames / bayesian_config.bin_size_time_offline
...
def load_and_prepare_session_for_bayesian_decoding(
    ...
    input_data: Literal["spks_pcs", "spks_all", "dff_all"] = "spks_pcs",
) -> (
    Tuple[
        Cached2pSession,
        np.ndarray,
    ...
    Returns:
        Cached2pSession:            The session object with the trials allotted according to the split strategy (decoder performance check only).
        np.ndarray:                 The session's OASIS deconvoluted place cell activity or data specified in 'input_data' handle, respectively.
        np.ndarray | None:          Place fields of the place cells in the array above (if 'pcs' in 'input_data' handle).
        list[TrialInfo]:            A list of the trials to test the decoder/a list of all trials if no split is applied.
        list[TrialInfo] | None:     A list of the trials which were used to get the place cells if a split is applied.
    ...
    dff, spks, _ = load_imaging_data(mouse=mouse_name, date=date)

    if "spks" in input_data:
        # TODO: double-check, but explicitly using BOD instead of speed thresholding now!
        if "pcs" in input_data:
            # returning pc activity
            t0 = time.time()
            ...
            if split_strategy is None:
                return session, place_cells, pcs_place_fields, trials_test, None
            else:
        ...
        activity = dff
    # returning all cells' spks/dff activity
    if split_strategy is None:
        return session, activity, None, trials_test, None
    else:
        return session, activity, None, trials_test, trials_train
...
def decode_events(
    ...
    for idx, event in enumerate(pse_activity):
        # "only bins with non-zero firing rates were used for offline Bayesian decoding."
        event = event[:, np.sum(event, axis=0) > 0]
...
def decode_task_immobility(
    ...
    for trial_i, trial_chunk_start_indices in enumerate(chunk_start_indices):
        for chunk_i, start in enumerate(trial_chunk_start_indices):
            ...
            ppm_path = SERVER_PATH / "viral_caches" / "bayesian" / "ppm" / INPUT_DATA
            if not os.path.exists(ppm_path):
                os.makedirs(ppm_path)
            np.save(ppm_path / f"{session.mouse_name}_{session.date}_ppm)_trial{trial_i}_chunk{chunk_i}.npy", posterior_probability_matrix)
            plot_ppm(
                grosmark_config=grosmark_config,
                session=session,
                posterior_probability_matrix=posterior_probability_matrix,
...
def decode_task_epoch(
    ...
    if len(pse_events) > 0:
        decoded_events = decode_events(
            ...
            # positions_test=ssp_test.position_vectors,
            significance=0.05,
            only_plot_significant_events=False,
...
def decode_wheel_freeze_epoch(
    ...
    """Perform the Bayesian decoding on either of the wheel freeze epochs of the session and plot the events."""
    ssp_config_mobility = SSPConfig(
        mode="above",
        speed_threshold=5,
        n_consecutive_samples=3 * 30,
...
def decode_for_performance_check(
    ...
    cache_path = (
        ...
        / "viral_caches"
        / "bayesian"
        / f"{mouse_name}_{date}_decoder_performance_CLIMER_bin_size_time-{temporal_bin_size}_bin_size_spatial_-{bayesian_config.bin_size_spatial}_{SPLIT_STRATEGY}_{INPUT_DATA}.npz"
    )
    ...
    if os.path.exists(cache_path) and use_cache:
        ...
        session, place_cells, _, trials_test, trials_train = (
        ...
        n_neurons = 50
        n_samples = 100
        dt = temporal_bin_size / 30  # e.g. 10 frames/bin / 30 frames/sec = 1/3 sec/bin
...
def main(
    ...
    # check that bin size and duration settings are not contradicting each other
    n_min_time_bins = (
        bayesian_config.event_duration[0] // bayesian_config.bin_size_time_offline
    )
    assert (
        n_min_time_bins >= 3
    ), f"A decoded event should have at least 3 time bins. Due to your settings, this could fail. Check!"
    ...
    if not prepared_session:
        return None
    session, place_cells, place_fields, trials_test, _ = prepared_session
    ...
    if use_cache and os.path.exists(cache_path):
        ...
        threshold_path = (
            ...
            / "viral_caches"
            / "bayesian"
            / f"{mouse_name}_{date}_task_pse_thresholds.npz"
            if bayesian_config.epoch in {"task", "pre", "post"}
            else f"{mouse_name}_{date}_{bayesian_config.epoch}_pse_thresholds.npz"
        )
        # phases of immobility
        """Offline immobility epochs were defined as those in which the animal's velocity,
        smoothed with a half-second Gaussian kernel, was below 3 cm s-1 for at least 3 consecutive seconds.
        ...
        if "task" in bayesian_config.epoch:
            ...
            if not session.wheel_freeze:
                ...
                return None
            pse_thresholds = load_pse_thresholds_session(threshold_path=threshold_path)
            result = decode_wheel_freeze_epoch(
                session=session,
                place_cells=place_cells,
                place_fields=place_fields,
                pse_thresholds=pse_thresholds,
...
# TODO: used to work with outdated code. is not of highest interest, maybe remove?
...
if __name__ == "__main__":
    ...
    bayesian_config_task = BayesianDecodingConfig(
        ...
        # event_duration=(30, 105), # last used
        event_duration=(15, 105),
        # event_duration=(6, 45),
        ...
        # bin_size_spatial=10,
        # bin_size_spatial=15,
        bin_size_spatial=10,  # originally 2 cms?
        n_samples=100,
        n_neurons=50,
    )
    grosmark_config = GrosmarkConfig(
    ...
    # CAUTION: two temporal bin sizes can be set, one for online, the other for offline decoding
    # thus, you should check the decoder's perfomance for both
    bayesian_config_decoder = copy.deepcopy(bayesian_config_task)
    bayesian_config_decoder.epoch = "task"

    INPUT_DATA: Literal["spks_pcs", "spks_all", "dff_all"] =  "spks_all" # "dff_all" # "spks_pcs"
    # how to split trials for training and testing the decoder
    # TODO: make this a real global variable
    ...
    print("Done with decoding check")
    for mouse_name in SESSIONS_KEEP.keys():
        metadata = gsheet2df(SPREADSHEET_ID, mouse_name, 1)
        for stage in ["unsupervised", "learning", "learned"]:
            print(f"Doing {mouse_name} at {stage} stage")
